...
from ASearcher.train.code_agent import CodeAgent
from ASearcher.utils.code_tool import CodeToolBox
from ASearcher.utils.rewards import em_check
import re

def correct_format_fn(idx, s):
    correct = all(
        [
            s.count("<python>") == s.count("</python>"),
            s.count("<answer>") == s.count("</answer>"),
            s.count("<python>") + s.count("<answer>") <= 1,
...
class ASearcherWorkflow(RolloutWorkflow):
    def __init__(
        ...
        self.toolbox = CodeToolBox()
    
    async def collect_agent_trajectory(self, qid, prompt, ground_truth, engine):
        ...
        score = 1 if em_check(pred_answer, ground_truth) else 0
